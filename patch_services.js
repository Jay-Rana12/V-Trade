...
// Using regex with non-greedy matches
let newContent = content.replace(/\\/\\*\s*══ NEW PRODUCT DETAIL MODAL ══\s*\\*\\/[\\s\\S]*?\\.pd-icon-btn:hover\s*\{[^}]+\}/, css_replacement);

newContent = newContent.replace(/<!-- ════════ PRODUCT DETAIL MODAL ════════ -->[\\s\\S]*?<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>/, html_replacement);

newContent = newContent.replace(/function switchTab\(btn, paneId\)[\\s\\S]*?function handleOverlayClick\(e\)[\\s\\S]*?\}/, js_replacement);

if (newContent === content) {
    console.log("No changes made. Pattern mismatch.");
} else {
