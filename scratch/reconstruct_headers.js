...
files.forEach(file => {
    ...
    try {
        ...
        // 2. Fix the duplicate style attribute and structure
        // We look for the whole search-bubble container and rebuild it cleanly
        const searchRegex = /<div class="search-bubble[^>]*>[\s\S]*?<\/div>\s*<\/div>/;
        ...
        // Find the start of the search-bubble and replace until the end of its block
        // This is tricky with regex, so we'll do a simpler replacement of the inner content
        
        if (content.includes('class="search-bubble')) {
            const parts = content.split(/<div class="search-bubble[^>]*>/);
            const prefix = parts[0];
            const rest = parts[1];
            const suffix = rest.substring(rest.indexOf('</div>\n                </div>') + 14); // Very specific to the known structure
            
            // Actually, let's just replace the whole header row for safety
...
`;
        ...
        console.log(`SUCCESS: ${file} header reconstructed.`);
    } catch (err) {
